"""PDF → PPTX 转换业务服务实现（零 Qt 依赖）。

服务通过注入的端口（PdfSlideConverter / EventEmitter）完成工作，
自身不 import 任何 GUI 库，也不直接依赖 pymupdf / python-pptx
（那些依赖被限制在 core/adapters 内）。

职责：
- 校验输出路径不得与模板/源 PDF 相同（保护源文件）；
- 委托适配器执行转换；
- 沿事件端口推送 PDF_PROGRESS / PDF_COMPLETED（core → UI 单向通道）。
"""
...
from core.services._path_guard import same_path
from core.ports.events import EventEmitter
from core.ports.io import PdfSlideConverter
from shared.contracts import (
    ConvertPdfRequest, ConvertPdfResult, EventType,
    PdfCompletedEvent, ProgressEvent,
)
from shared.errors import OutputOverwriteError


class PdfSlideServiceImpl:
    """PDF → PPTX 转换服务：校验路径 → 转换 → 推送进度/完成事件。"""

    def __init__(self, converter: PdfSlideConverter, emitter: EventEmitter) -> None:
    ...
    def convert(self, request: ConvertPdfRequest) -> ConvertPdfResult:
        ...
        if same_path(request.pdf_path, request.output_path):
            raise OutputOverwriteError(
        ...
        self._emitter.emit(
            ProgressEvent(
                type=EventType.PDF_PROGRESS, message="准备中...", current=0, total=0
            )
        )
        ...
        self._emitter.emit(PdfCompletedEvent(result=result))
        return result
