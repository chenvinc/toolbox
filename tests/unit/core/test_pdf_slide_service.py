"""PdfSlideServiceImpl 单元测试（无 Qt、无真实文件 IO）。

用 FakePdfSlideConverter + CollectingEmitter 注入端口，验证：
...
from shared.contracts import ConvertPdfRequest, ConvertPdfResult, EventType
...
class FakePdfSlideConverter:
    """实现 PdfSlideConverter 端口：模拟 3 页 PDF，逐页回调进度。"""
...
class PdfSlideServiceTests(unittest.TestCase):
    def setUp(self):
        self.emitter = CollectingEmitter()
        self.converter = FakePdfSlideConverter()
        self.svc = PdfSlideServiceImpl(self.converter, self.emitter)

    def test_convert_returns_result_and_calls_converter(self):
        res = self.svc.convert(_req())
        self.assertEqual(res.page_count, 3)
        self.assertEqual(res.textbox_count, 10)
        self.assertEqual(res.run_count, 20)
    ...
    def test_convert_emits_progress_and_completed_events(self):
        ...
        # 准备中 + 3 页进度 + 转换完成 + 完成事件
        self.assertEqual(types.count(EventType.PDF_PROGRESS), 5)
        self.assertEqual(types[-1], EventType.PDF_COMPLETED)
    ...
    def test_progress_events_carry_page_numbers(self):
        ...
        page_events = [
            ...
            if e.type == EventType.PDF_PROGRESS and e.total == 3
        ]
    ...
    def test_output_same_as_template_raises(self):
        with self.assertRaises(OutputOverwriteError):
            self.svc.convert(_req(template_path="same.pptx", output_path="same.pptx"))
        self.assertEqual(self.converter.calls, [])

    def test_output_same_as_pdf_raises(self):
        with self.assertRaises(OutputOverwriteError):
            self.svc.convert(_req(pdf_path="same.pptx", output_path="same.pptx"))
        self.assertEqual(self.converter.calls, [])

    def test_container_builds_pdf_slide_service(self):
        c = Container.build(
            task_runner=SyncTaskRunner(), event_emitter=CollectingEmitter()
        )
        self.assertIsInstance(c.resolve("pdf_slide"), PdfSlideServiceImpl)
