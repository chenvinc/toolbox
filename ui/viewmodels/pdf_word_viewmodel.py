...
from shared.contracts import (
    ConvertPdfToWordRequest, DomainEvent, EventType,
    ProgressEvent, WordCompletedEvent, WordFailedEvent,
)
...
class PdfWordViewModel(BaseViewModel):
    ...
    def _dispatch(self, event: DomainEvent) -> None:
        ...
        if isinstance(event, ProgressEvent):  # WORD_PROGRESS
            self.progress.emit(event.message, event.current, event.total)
        elif isinstance(event, WordCompletedEvent):
            self.completed.emit(event.result)
        elif isinstance(event, WordFailedEvent):
