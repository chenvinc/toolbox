"""PDF → Slide 视图模型（胶水层，仅做命令转发与事件→信号桥接）。

业务规则一律在 core/services 内；本类不解析 PDF、不生成 PPT，
只负责：把 UI 的 ConvertPdfRequest 转发给 service，并把 core 推送的
...
from core.ports.services import PdfSlideService
from core.ports.tasks import TaskRunner
from shared.contracts import (
    ConvertPdfRequest, DomainEvent, EventType,
    PdfCompletedEvent, PdfFailedEvent, ProgressEvent,
)
...
class PdfSlideViewModel(BaseViewModel):
    """Pdf2Slide 对应的视图模型。"""

    # ── UI 可绑定的信号 ──
    progress = Signal(str, int, int)   # message, current, total
    completed = Signal(object)         # ConvertPdfResult
    failed = Signal(object)             # str(消息) 或 异常对象（见 on_async_error）

    _WATCHED = frozenset({
        EventType.PDF_PROGRESS,
        EventType.PDF_COMPLETED,
        EventType.PDF_FAILED,
    })

    def __init__(
        self,
        pdf_slide: PdfSlideService,
        task_runner: TaskRunner,
        event_emitter: EventEmitter,
    ) -> None:
        super().__init__(event_emitter, task_runner)
        self._pdf_slide = pdf_slide
    ...
    def _dispatch(self, event: DomainEvent) -> None:
        ...
        if isinstance(event, ProgressEvent):  # PDF_PROGRESS
            self.progress.emit(event.message, event.current, event.total)
        elif isinstance(event, PdfCompletedEvent):
            self.completed.emit(event.result)
        elif isinstance(event, PdfFailedEvent):
            self.failed.emit(event.message)

    # ── 命令转发（单向数据流：UI → core） ──
    @async_task
    def convert(self, request: ConvertPdfRequest) -> Any:
        return self._pdf_slide.convert(request)
