...
当前内置 5 个工具（Tool）（v4.0 原为 4 个，Pdf2Word 为后续新增）：
...
| `Pdf2Word` | 📄 PDF转Word | PDF → 保留可编辑文字（流式段落）的 Word 文档 |
...
│     │                                                                  │
│     ├── views/  BaseView + 5 个 View（仅渲染/事件绑定）                │
│     │       └── 订阅 ViewModel 的 Signal                              │
│     ├── viewmodels/  5 个 ViewModel（胶水：命令转发 + 事件→信号）      │
...
│     ▼                                    │   services.py / io.py      │
│  adapters/  5 个适配器（封装 python-docx / pptx / pymupdf）             │
│                                        │   tasks.py / events.py       │
...
│   └── di.py              # 依赖注入容器
├── ui/
│   ├── views/             # BaseView + 5 个 View
│   ├── viewmodels/        # 5 个 ViewModel（胶水层）
│   └── infra/             # QtTaskRunner · QtEventEmitter · preview_escape
...
| 文件 | Protocol | 关键方法签名 |
| --- | --- | --- |
| `services.py` | `SimilarityService` / `ExtractionService` / `PptxService` / `PdfSlideService` / `ExamGeneratorService` / `PdfWordService` | `check(request)` / `extract(request)` / `generate(request)` / `convert(request)` |
| `io.py` | `DocumentLoader` / `PptxWriter` / `PdfSlideConverter` / `ExamDocxWriter` / `PdfWordConverter` | `load_paragraphs(path)` / `build(...)` / `convert(request, on_progress)` |
...
#### 3.2.3 `core/adapters/`（5 个，封装第三方库）
| 适配器 | 文件 | 实现的端口 | 第三方库 |
| --- | --- | --- | --- |
...
| `PdfWordConverterAdapter` | `pdf_word_converter.py` | `PdfWordConverter` | PyMuPDF + python-docx |

#### 3.2.4 `core/services/`（6 个服务实现）
| 服务 | 文件 | 端口 | 关键行为 |
| --- | --- | --- | --- |
...
| `PdfWordServiceImpl` | `pdf_word_service.py` | `PdfWordService` | 校验（输出≠模板/源）→ `converter.convert` → `WordCompletedEvent` |

> ~~⚠️ `core/services/__init__.py` **导出不完整**：仅导出 `SimilarityServiceImpl / ExtractionServiceImpl / PptxServiceImpl`，漏了 `JsonToWordServiceImpl` 与 `PdfSlideServiceImpl`。~~ ✅ **已修复（v4.0）**：`__init__.py` 已补齐全部 5 个 Service 导出（`SimilarityServiceImpl` / `ExtractionServiceImpl` / `PptxServiceImpl` / `JsonToWordServiceImpl` / `PdfSlideServiceImpl`，含 `__all__`），与 §8 #1 一致。新增服务时仍应同步更新导出。

#### 3.2.5 `core/di.py`
`Container.build(*, task_runner, event_emitter) -> Container`：
- 实例化 5 个适配器 → 6 个服务（注入 loader/writer/emitter）。
...
#### 3.2.6 `PdfWordConverterAdapter` 段落重建算法（适配器内）

`convert()` 不是「逐视觉行 → 逐段落」，而是把 PDF 文本重建成**语义段落**。动机：PDF 里一段中文常因版式换行被切成多个视觉行，若 1 行 1 段落，中文段落会被切碎，且 CJK 无词间空格会导致英文跨行连写（`word`+`across` → `wordacross`）。算法两阶段，配套数据类 `_Span` / `_Line` 与辅助函数 `_is_cjk` / `_join_sep` / `_should_merge` / `_write_paragraph`。

**阶段一 · 块内合并（block-internal）**
PyMuPDF 的 `block`（`type==0`）本身是连贯文本区，块内多行只是同一段落的视觉换行，故块内所有行**无条件**合并为一个候选段落。

**阶段二 · 块间智能合并（smart merge）**
相邻候选段落需**同时满足**以下三条才合并为同一语义段落（见 `_should_merge`）：

1. **垂直间距 ≤ 1.3× 行高中位数**：取整页各行高 `y1−y0` 的中位数 `median_h`，上一块末行底 `y1` 到 下一块首行顶 `y0` 的间距 `gap`（负值夹 0）须 `≤ 1.3·median_h`；
2. **无首行缩进跳变**：`下块首行 x0 − 上块首行 x0 < 0.5×字号`，否则视为新段落（首行缩进）；
3. **字号未显著突变**：两首行字号比值差 `≤ 25%`（`abs(a−b)/max(a,b) ≤ 0.25`），避免把标题并入正文。

**行间接续规则（connection rule，`_join_sep`）**
相邻视觉行拼接处补不补空格：

- 上一行末字符 **或** 下一行首字符任一侧为 CJK / 中文标点（`_is_cjk` 覆盖 `0x3000–0x303F`、`0x3400–0x4DBF`、`0x4E00–0x9FFF`、`0xF900–0xFAFF`、`0xFF00–0xFFEF`、`0x3040–0x30FF`）→ **不补空格**；
- 两侧皆拉丁字母 / 数字 → **补一个空格**（修复英文跨行连写）。

**run 级格式**
每行 span 仍作为独立 Word run 写入，字体/字号/颜色/粗斜体经 `_set_run_format` 原样保留（`w:rFonts` 的 `eastAsia` 确保中文按 PDF 所用中文字体渲染）；行间接续空格单独成 run（借用下一行首 span 格式，空格本身不可见）。

**已知边界（v4.x 暂不做，列为后续增强）**

- 仅按 `(y0, x0)` 排序近似**单栏**阅读顺序，未做多栏检测；多栏 PDF 会顺序交错。
- 不做**段落版式还原**（首行缩进、对齐、段间距等），仅保证文字连续与格式保留。

**验证**：`tests/unit/core/test_pdf_word_e2e.py` 用真实 PyMuPDF / python-docx 覆盖——中文多视觉行合并为单一段落且无 ASCII 空格、英文跨行补空格无连写、模板复用清空原有正文、粗体红色保留、非法 PDF 抛 `PdfReadError`。
...
- 所有 ViewModel 继承 `BaseViewModel`（v4.0 抽取，内含 `_WATCHED` 串台防护、`cancel_current` 与事件订阅），不再各自复制模板（见 §8 #2 与[开发指南](./新增Tool开发指南.md) §5）。
- 每个 VM 持有 service + `task_runner` + `event_emitter`，定义业务 `Signal`，并实现 `on_async_error` / `cancel_current` / `_on_event` 与 `@async_task` 命令方法。
- ~~⚠️ **信号载荷不一致**：`JsonExamViewModel.failed` 是 `Signal(object)`，其余 3 个发 `str(exc)`。~~ ✅ **已修复（v4.0，Phase 2）**：五个 VM 的失败信号统一为 `Signal(object)`，`on_async_error` 透传异常对象（以 `JsonExamViewModel` 为范本），视图侧统一 `msg = message if isinstance(message, str) else str(message)` 处理（与 §8 #5 一致）。
- ⚠️ **共享 emitter 陷阱（结构性约定，非缺陷）**：5 个 VM 共用同一个 `QtEventEmitter` 实例，全部 `_on_event` 都会收到所有事件，靠 `EventType` 过滤。**新增工具必须新增专属 `EventType`**，否则会串台。`SlideViewModel` 曾把 `CHECK_FAILED` 也映射到 `pptx_failed` 属历史耦合，✅ **已解耦（v4.0，Phase 2）**：`SlideViewModel` 不再监听 `CHECK_FAILED`（`CHECK_*` 属 SimilarityChecker），与 §8 #4 一致。本 ⚠️ 现仅作为新增工具的规约提醒保留。
...
### 4.3 工具注册
`app.py`：`Container.build` → 造 5 个 VM → `_register_tools(...)` → 逐个 `_add_tool(View)`。
`_add_tool` 用 `get_nav_title()` 作导航项文本、`f"{get_name()}\n{get_description()}"` 作 tooltip。
`_on_nav_changed` 切换时调 `tool.on_activate()`（当前 5 个 View 均未重写，为空实现）。
...
| Pdf2Word | `PdfWordServiceImpl` | `PdfWordConverterAdapter` | `PdfWordViewModel` | `PdfWordView` | `ConvertPdfToWordRequest`/`Result` |

---

## 6. 视图通用规范（UI 一致性）

5 个 View 高度同构，新视图应以 `SimilarityView` 为范本（其 `_restyle_all` 最完整）。通用模式：

1. **构造顺序**（`__init__`）：
   `self._vm = view_model` → `super().__init__()`（`BaseView` 统一初始化 `self.theme` 与三样式列表；子类**无需**手写 `self.theme = Theme()`）→ 业务状态字段 → `self.settings = QSettings("<AppName>", "<AppName>")`（两参同名）→ `_setup_ui()` → `_connect_view_model()` → `_load_settings()` → `self.theme.theme_changed.connect(self._on_theme_changed)`。
...
7. **`stop_worker()`**：5 个 View 均实现为 `self._vm.cancel_current()`（1 行）。`on_activate()` 均未重写。
...
> ⚠️ **已知不一致（供重构参考）**：① ~~SlideView 的 `_restyle_all` 漏刷 DropZone 与 `change_btn`~~ ✅ **已修复（v4.0，已对齐 SimilarityView）**；② 同一概念「题号格式」在 SlideView 存 `question_num_fmt`，SimilarityView 存 `num_pattern`；③ ~~`first_line_indent` 在 QSettings 中以**字符串** `"true"/"false"` 存储，读取时 `== "true"` 比较~~ ✅ **已修复（v4.0，Phase 3）**：`json_exam_view` 改为真布尔存储（`isChecked()`/`setChecked()`），读取兼容旧 `"true"/"false"` 字符串值（`raw if isinstance(raw, bool) else str(raw) == "true"`），与 §8 #9 一致；④ ~~三种「打开文件夹」实现（`subprocess` / `os.system` / 兼容 `folder:` 前缀）不统一~~ ✅ **已修复（v4.0，Phase 2，已抽 `ui/infra/open_folder.py`）**；⑤ 部分设计令牌（`spacing`/`page_pad_*`/`font_*`）在 View 中未被引用，布局仍是硬编码。
...
> ⚠️ **控件坑（仅余 1 项仍开放）**：① ~~`AppButton.setEnabled` 被重写为 `set_actionable(enabled, "")`，会清空已设禁用原因~~ ✅ **已修复（v4.0）**：`AppButton` 不再重写 `setEnabled`，改用 `changeEvent`（`QEvent.EnabledChange`）被动同步视觉态，禁用原因由 `set_actionable` 专管，外部直接 `setEnabled(False)` 不会清空既有原因；② ~~`StepperInput` 在 `QSpinBox` 模式下 `valueChanged` **永不发射**~~ ✅ **已修复（v4.0）**：已桥接 `self._spin.valueChanged.connect(self._relay_value)`，两种模式（SpinBox / QLineEdit）对外均发射 `valueChanged(float)`；③ ~~`DropZone`/`MultiDropZone` 的 `theme=None` 会 `AttributeError`~~ ✅ **已修复（v4.0）**：两者构造均 `theme if theme is not None else Theme()` 回退全局单例，签名与实现一致；④ `MultiDropZone` 没有 `file_cleared` 信号——**刻意设计**（清空统一经 `files_selected([])` 表达，避免冗余 API），非缺陷，保持现状。
...
| 2 | ViewModel 无基类、4 份复制 | ✅ **已修复（v4.0）**：抽 `BaseViewModel`，内置 `_WATCHED` 串台防护，5 个 VM 继承之 |
...
| 5 | `failed` 信号载荷不一致（str vs object） | ✅ **已修复（v4.0，Phase 2）**：五个 VM 的失败信号统一为 `Signal(object)`，`on_async_error` 透传异常对象（以 `JsonExamViewModel` 为范本），视图侧统一 `msg = message if isinstance(message, str) else str(message)` 处理 |
| 6 | `Theme` 非单例、≥5 实例 | ✅ **已修复（v4.0，Phase 4）**：`Theme` 改为全局单例（`__new__` 缓存 + `_initialized` 守卫），升级为 `QObject` 暴露 `theme_changed` 信号；`refresh()` 末尾广播该信号，`app.py` 保留唯一 `colorSchemeChanged` 接线触发 `refresh()`，5 个 View 改订阅 `self.theme.theme_changed`（处理器去掉 `refresh()` 防自触发死循环）。OS 级连接由 5 处降为 1 处 |
